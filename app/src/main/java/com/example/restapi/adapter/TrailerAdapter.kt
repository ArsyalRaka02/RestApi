...
class TrailerAdapter (c: FragmentActivity?, data: List<Trailers.Data>?): RecyclerView.Adapter<TrailerAdapter.ViewHolder>() {
    ...
    private fun watchYoutube(id: String?){
        val appIntent: Intent = Intent(Intent.ACTION_VIEW, Uri.parse("vnd:$id"))
        val webIntent: Intent = Intent(Intent.ACTION_VIEW, Uri.parse("http://wwww.youtube.com/watch?v=$id"))
